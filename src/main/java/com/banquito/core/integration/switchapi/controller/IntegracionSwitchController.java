...
public class IntegracionSwitchController {
    ...
    public ApiResponse<?> validarEmpresa(@PathVariable String ruc) { return ApiResponse.ok("Validacion de empresa", service.validarEmpresa(ruc)); }

    @GetMapping("/cuentas/{numeroCuenta}/disponibilidad")
    public ApiResponse<?> disponibilidad(@PathVariable String numeroCuenta) { return ApiResponse.ok("Disponibilidad de cuenta", service.consultarDisponibilidad(numeroCuenta)); }

    @PostMapping("/transacciones/transferencia")
    public ApiResponse<?> transferencia(@Valid @RequestBody TransferenciaRequest request) { return ApiResponse.ok("Transferencia Switch procesada", service.ejecutarTransferencia(request)); }

    @PostMapping("/transacciones/liquidacion-servicio")
    public ApiResponse<?> liquidacion(@Valid @RequestBody LiquidacionServicioSwitchRequest request) { return ApiResponse.ok("Liquidacion procesada", service.liquidarServicio(request)); }
}
