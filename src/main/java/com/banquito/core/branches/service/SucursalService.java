...
public class SucursalService {
    ...
    public List<SucursalResponse> listar() { return repository.findAll().stream().map(SucursalMapper::toResponse).toList(); }
    public Sucursal obtenerEntidad(Integer id) { return repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Sucursal no encontrada: " + id)); }
    public SucursalResponse obtener(Integer id) { return SucursalMapper.toResponse(obtenerEntidad(id)); }
